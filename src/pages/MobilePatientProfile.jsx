...
import {
  ...
  Stethoscope, Receipt, X, Check, Camera
} from 'lucide-react';
...
export default function MobilePatientProfile() {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const [patRes, plansRes, paysRes, apptRes, usersRes, toothRes] = await Promise.all([
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#F3F6F8] flex flex-col">
      <div className="px-4 pb-8" style={{ paddingTop: 'max(12px, env(safe-area-inset-top, 12px))', background: 'linear-gradient(165deg, #0B6F7C 0%, #1499AD 52%, #2BB8C8 100%)' }}>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(165deg, #0A6A76 0%, #1499AD 48%, #2BB8C8 100%)',
      ...
      <div className="px-3 -mt-6 grid grid-cols-3 gap-2 relative z-10">
        ...
        >
          <div className="w-11 h-11 rounded-2xl bg-cyan-50 border border-cyan-100 flex items-center justify-center">
            <ClipboardList className="w-5 h-5 text-[#1499AD]" />
          </div>
      ...
      <div className="flex-1 px-3 pt-3 pb-2">
        <AnimatePresence mode="wait">
          <motion.div key={activeTab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.16 }}>
              ...
              <div className="space-y-3">
                <div className="bg-white rounded-[20px] shadow-[0_8px_24px_rgba(15,23,42,0.05)] border border-slate-100/80 px-3 pt-3 pb-3.5">
                  <div className="flex items-center justify-between mb-2.5 px-0.5">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-wide text-[#1499AD]">
                ...
                {selectedTooth ? (
                  <div className="bg-white rounded-[20px] shadow-[0_8px_24px_rgba(15,23,42,0.05)] border border-slate-100/80 p-3.5">
                    <div className="flex items-start gap-3 mb-3">
                      <div className="w-11 h-11 rounded-xl bg-[#1499AD] text-white flex items-center justify-center text-[13px] font-black shrink-0 shadow-sm">
                        #{selectedTooth.fdi}
                      </div>
                      ...
                      <div className="space-y-2.5 mb-3.5 pl-1">
                        {previewHistory.map((item) => (
                          <div key={item.id} className="flex items-start gap-2.5">
                            <span className={cn('mt-1.5 w-1.5 h-1.5 rounded-full shrink-0', item.highlight ? 'bg-rose-500' : 'bg-[#1499AD]')} />
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        ...
                        className="h-11 rounded-2xl border-2 border-[#1499AD]/40 text-[#1499AD] text-[12px] font-black active:scale-[0.98] transition-transform"
                      >
                    ...
                    <div key={plan.id} className="bg-white rounded-2xl border border-slate-100 shadow-xs overflow-hidden">
                      <button className="w-full p-4 text-left flex items-start gap-3 active:bg-slate-50 transition-colors" onClick={() => setExpandedPlan(isExpanded ? null : plan.id)}>
                        <div className="w-10 h-10 rounded-xl bg-cyan-50 border border-cyan-100 flex items-center justify-center shrink-0">
                          <Stethoscope className="w-5 h-5 text-[#1499AD]" />
                        </div>
                          ...
                          {services.length > 0 && (
                            <div className="mt-2.5">
                              ...
                              <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                <div className="h-full rounded-full transition-all" style={{ width: `${progress}%`, background: 'linear-gradient(90deg, #2BB8C8, #1499AD)' }} />
                              </div>
                            </div>
                          )}
                        </div>
                ...
                {appointments.length === 0 ? (
                  ...
                  <div key={appt.id} className="bg-white rounded-2xl border border-slate-100 shadow-xs p-4 flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p className="text-sm font-black text-slate-900">{appt.title || appt.reason || t('patientProfile.mobile.apptAction', 'Uchrashuv')}</p>
                      ...
                      <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                        {appt.date && (
                          <span className="text-[10px] font-bold text-slate-500 flex items-center gap-1">
                            ...
                            {new Date(appt.date).toLocaleDateString(dateLocale, { day: '2-digit', month: 'short', year: 'numeric' })}
                          </span>
                        )}
                        {appt.time && (
                          <span className="text-[10px] font-bold text-slate-500 flex items-center gap-1">
            ...
            {activeTab === 'notes' && (
              <div className="space-y-3">
                <h2 className="text-sm font-black text-slate-800">{t('patientProfile.tabs.notes', 'Eslatmalar')}</h2>

                <div className="bg-white rounded-2xl border border-slate-100 shadow-xs p-4 space-y-3">
                  <p className="text-xs font-bold text-slate-500">{t('patientProfile.mobile.newEntry', "Yangi eslatma qo'shish")}</p>
                  <textarea
                    value={noteText}
                    onChange={e => setNoteText(e.target.value)}
                    placeholder={t('patientProfile.patientInfoCard.note', 'Eslatma yozing...')}
                    rows={3}
                    className="w-full rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-800 font-medium resize-none focus:outline-none focus:ring-2 focus:ring-[#1499AD]/30 focus:border-[#1499AD] transition-all placeholder:text-slate-400"
                  />
                  <button
                ...
                {patient.notes ? (
                  <div className="bg-white rounded-2xl border border-slate-100 shadow-xs p-4 space-y-2">
                    <p className="text-xs font-bold text-slate-500 uppercase mb-3">{t('patientProfile.tabs.notes', 'Eslatmalar')}</p>
                  ...
                  <div className="bg-white rounded-2xl border border-slate-100 p-10 text-center shadow-xs">
                    ...
                    <p className="text-slate-500 font-bold text-sm">{t('patientProfile.mobile.emptyNotes', "Eslatmalar yo'q")}</p>
                  </div>
                )}
                ...
                {patient.phone && (
                  <button
                    type="button"
                    onClick={() => { navigator.clipboard.writeText(patient.phone); toast.success('Nusxalandi'); }}
                    className="w-full bg-white rounded-2xl border border-slate-100 p-3.5 flex items-center gap-3 text-left"
                  >
                    <Phone className="w-4 h-4 text-[#1499AD]" />
                    <span className="flex-1 text-sm font-bold text-slate-800 font-mono">{formatPhone(patient.phone)}</span>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive ? 'bg-[#E6F6F8] text-[#0E7A8A]' : 'text-slate-400'
                )}
              >
                {tab.label}
              </button>
                ...
                <div className="relative pl-3 space-y-3 before:absolute before:left-[7px] before:top-1 before:bottom-1 before:w-px before:bg-slate-200 pb-2">
                  {selectedHistory.map((item) => (
                    <div key={item.id} className="relative pl-4">
                      <span className={cn('absolute left-0 top-1.5 w-2.5 h-2.5 rounded-full border-2 border-white shadow-sm', item.highlight ? 'bg-rose-500' : 'bg-[#1499AD]')} />
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <input type="number" inputMode="numeric" value={payForm.amount} onChange={e => setPayForm(p => ({ ...p, amount: e.target.value }))}
                    placeholder="0"
                    className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-xl font-black text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#1499AD]/30 focus:border-[#1499AD] transition-all" />
                </div>
                ...
                <div className="flex gap-2">
                  {[
                    ...
                    <button key={v} onClick={() => setPayForm(p => ({ ...p, method: v }))}
                      className={cn('flex-1 py-2.5 rounded-xl text-xs font-bold border transition-all active:scale-95',
                        payForm.method === v ? 'bg-cyan-50 border-[#1499AD] text-[#0E7A8A]' : 'bg-slate-50 border-slate-200 text-slate-600')}>
                      {l}
                    </button>
                  ))}
                </div>
                ...
                <div>
                  ...
                  <input type="text" value={payForm.notes} onChange={e => setPayForm(p => ({ ...p, notes: e.target.value }))}
                    ...
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#1499AD]/30 focus:border-[#1499AD] transition-all" />
                </div>
