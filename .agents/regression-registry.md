...
- **Qanday tuzatildi:** Chart karta kengligining 100% i: yuqori 18→11 | 21→28, pastki 48→41 | 31→38. Har bir tish `minmax(0, 1fr)`. Holat PNG lari (`endo`, `caries`, `plomba`, `implant`, `missing`, `healthy`) o'z joyida. Sog'lom tish `/teeth/healthy/{fdi}.png` (ivory). Implant xira filtrsiz.
...
- **Qanday tuzatildi:** Yangi service worker faollashganda ochiq oynalarni bir marta yangilaydi (`sw-activate-reload.js`). Sahifa `controllerchange` da qayta yuklanadi. Offline precache (`navigateFallback: index.html`) saqlanadi. Tish paneli `activeFdi` bo'yicha ochiladi. Tanlangan tishlar `data-stack="down"` ustunida, bosish tartibida, «+ Yangi bemor» ostida va chart pastida yig'iladi. Qatorlar inline `transform: none`.
- **Qaytalamaslik choralari:** Activate-reload skriptini olib tashlamang, aks holda ochiq PWA yana eski wizardda qoladi. Tanlangan tishlar ro'yxati `flex-direction: column` va `data-stack="down"` bo'lib qolsin — gorizontal pill yoki faqat «N ta tish» hisobiga qaytarmang. Wizard chartga `rotate` / `archT` qaytarmang. `onToggle` ni `focusTooth` dan uzmang. Step 3 overlay va shifokor-majburiy bemor oqimi o'zgarmasin. `node scripts/assert-implant-wizard-ux.mjs` o'tsin.
